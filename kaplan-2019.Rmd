...
The 95% confidence interval on the effect size is simply expressed in terms of F:
...
Many statistical educators believe -- rightly or wrongly -- that drill with hand computation of quantities such as the mean and standard deviation informs a student's understanding of these quantities. Such instructors may be inclined to believe that "computation by eye," as can be done with the graphics (a) through (d), is similarly beneficial.
...
The formula for F is somewhat tedious, but can be readily approximated from the graph in Figure 4.

```{r F-graph, echo = FALSE, fig.cap="Computing F from R and n"}
make_vals <- function(n) {
  tibble::data_frame(R = seq(0.05, 0.95, length = 100), 
             samp_size = n,
             F = (n - 1) * R^2 / (1 - R^2),
             ci.lower = 1 - 2 / sqrt(F),
             ci.upper = 1 + 2 / sqrt(F),
...
gf_line(F ~ R, color = ~ samp_size, data = Vals) %>%
...
```{r ci-from-F, echo = FALSE}
gf_ribbon(ci.lower + ci.upper ~ R, fill = ~ samp_size, data = Vals, 
          alpha = 0.9) +
  scale_x_continuous(breaks = seq(0.1, 0.9, by = 0.1)) +
  scale_y_continuous(
    breaks = seq(-1, 3, by = 1),
...
1. Constant 2 in the formula for CI doesn't handle small n. Using 4 for the F value equivalent to $p < 0.05$ isn't even roughly right for small n.
2. Why not use t? One-tailed versus two-tailed tests.
3. It's not easier than using the standard error.
