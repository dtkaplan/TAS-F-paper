...
The paper introduces a new approach to teaching inference in college-level introductory statistics that can be used to streamline the inference curriculum and unify the various settings distinguished in the traditional curriculum. The new approach may be especially valuable to instructors who want to include appropriate statistics for handling covariates. The approach is appropriate even for instructors who avoid using computing in their courses, but can also be used by instructors who embrace computing for multiple regression or for a bootstrapping/simulation pedagogy. 

# Introduction

\LARGE

A conventional approach to teaching statistical inference in a first university-level course refers to a series of statistics and their standard errors in several specific contexts: the difference between two proportions, the difference between two means, and the slope of a regression line. Prior to this, the concept of a standard error is introduced in the context of a single sample proportion or sample mean. 

I'll refer to the conventional (or "traditional") curriculum as the "SE curriculum." This paper presents a new and different perspective on introducing inference that unifies the various settings of the SE curriculum, potentially allowing instructors to streamline their courses and thereby extend the range of topics to include multivariable settings, such as recommended by the 2016 GAISE College report. [Cite: GAISE]

The challenges faced by students in the SE curriculum are well known to instructors: the formulas for the standard errors are complicated; the formulas are different for the various settings yet similar enough to be mistaken for one another; the connection of the formulas to the underlying idea of sampling variation is not obvious; the concept of a sampling distribution is nuanced and difficult to assimilate; and cognitive load is imposed by the repeated use of the words sample, standard, error, statistic in the vocabulary: standard deviation, standard error, margin of error, sample statistic, sampling variation, test statistic.
...
An important inferential statistic based on R^2^ and sample size $n$ is the F-statistic. For the settings (a) through (d), which all have one degree of freedom, the F statistic g;ven in Eq. 2. (When there is more than one degree of freedom, see Eq. 4.)
...
Pboth <- 
  ...
  gf_theme(our_theme) %>%
  gf_segment(wage + wage ~ 9.5 + 9.7, shape = "-", alpha = 0.5, width = 2) %>%
  gf_pointrange(mean + lower + upper ~ 9.4, 
                data = Raw_stats_sector,  color = "red", size = 1.5, alpha = 0.5) %>%
  gf_errorbar(mean + mean ~ sector, color = ~ sex, data = Stats_sector, position = position_dodge(), size = 2) %>%
gf_segment(mean + mean ~ 8.8 + 9.0, shape = "-", color = ~ sex,
           data = Stats_sector, size = 1.5,
           alpha = 0.5, width = 2) %>%
  gf_pointrange(middle + lower + upper ~ 9.1, 
                data = Mod_stats,  color = "blue", size = 1.5, alpha = 0.5)
Pboth
```



The form of F changes slightly to incorporate the multiple degrees of freedom. For example, when there are k groups defined by the categorical variable, F is given by Eq. 4. 
...
1. to simplify the topic of inference while keeping it genuine by reducing the several settings of the traditional approach to a single setting that can be handled by a single graphical format.
...
Putting aside the objections to the RF approach that will necessarily be fostered of lack of experience with it, let's now some potential statistical-theory related criticisms.
